...
open_seir_step <- function(S, E, I, R, N, Beta, mu_IR, mu_L, delta.t){
  SI_events <- rbinom(n = 1, size = S, prob = 1 - exp((-mu_L -Beta*(I/N))*delta.t))
  SI_deaths <- rbinom(n = 1, size = SI_events, prob = 1 - exp(-mu_L/(mu_L +Beta*(I/N))))
...
open_seir_wrapper <-  function(N, I0, R0, Beta, mu_IR, mu_L, time, ...){
  # set up
  S0 <- N - I0  - R0
  time.steps <- (time - dplyr::lag(time))
  # init 
  ret <- matrix(NA, nrow = length(time.steps) , ncol = 4)
  ret[1,] <- open_seir_step(S = S0, 
                            I = I0, R = R0, N = N, 
                            Beta = Beta, 
                            mu_IR = mu_IR, 
                            mu_L = mu_L,
...
# Closed Model
#..............................................................
closed_seir_step <- function(S,I, R, N, Beta, mu_IR, delta.t){
...
# recursively just step through time now
closed_seir_wrapper <-  function(N, I0, R0, Beta, mu_IR, time, ...){
  # set up
  S0 <- N - I0 - R0
  time.steps <- (time - dplyr::lag(time))
  # init 
  ret <- matrix(NA, nrow = length(time.steps) , ncol = 3)
  ret[1,] <- closed_seir_step(S = S0, 
                              I = I0, R = R0, N = N, 
                              Beta = Beta, 
...
renderPlot({ withProgress(message = 'Making plot', {
  ...
  cum_closed_infected_plot <- closed_truth %>% 
    tidyr::gather(., key = "compartment", "count", 2:ncol(.)) %>% 
    dplyr::mutate(Compartment = factor(compartment, levels = c("S", "I", "R"))) %>% 
    dplyr::filter(Compartment == "I") %>% 
    dplyr::mutate(cumInxfn = cumsum(count)) %>% 
    ggplot() + 
    geom_line(aes(x = time, y = cumInxfn), color = "#e41a1c", size = 3) +
    geom_area(aes(x = time, y = cumInxfn), fill = "#e41a1c", alpha = 0.5) +
    xlab("Time") + ylab("Num. Infxn") + ggtitle("SIR Closed Population") +
    scale_y_continuous(label = scales::unit_format(big.mark = ",", scale = 1, unit = "")) +
    theme_bw() +
    theme(
      plot.title = element_text(family = "Helvetica", face = "bold", vjust = 0.5, hjust = 0.5, size = 18),
  ...
  cum_open_infected_plot <- open_truth %>% 
    tidyr::gather(., key = "compartment", "count", 2:ncol(.)) %>% 
    dplyr::mutate(Compartment = factor(compartment, levels = c("S", "I", "R"))) %>% 
    dplyr::filter(Compartment == "I") %>% 
    dplyr::mutate(cumInxfn = cumsum(count)) %>% 
    ...
    geom_line(aes(x = time, y = cumInxfn), color = "#e41a1c", size = 3) +
    geom_area(aes(x = time, y = cumInxfn), fill = "#e41a1c", alpha = 0.5) +
    xlab("Time") + ylab("Num. Infxn") + ggtitle("SIR Open Population") +
    scale_y_continuous(label = scales::unit_format(big.mark = ",", scale = 1, unit = "")) +
    theme_bw() +
    theme(
      plot.title = element_text(family = "Helvetica", face = "bold", vjust = 0.5, hjust = 0.5, size = 18),
  ...
  # plot out
  #..............................................................
  toprow <- cowplot::plot_grid(closed_seir_plot, open_truthPlot, nrow = 1)
  #middlerow <- cowplot::plot_grid(cum_closed_infected_plot, cum_open_infected_plot, nrow = 1)
  
  
  # R0
  R0 <- round(input$beta / (input$mu_IR + input$mu_L), 2)
  bottomrow <- ggplot() + geom_text(aes(0,0,
              label= paste0("R[0] %~~% beta / (gamma + mu[L]) %~~%", R0)), parse=TRUE, size = 10, fontface = "bold") +
    theme_minimal() + 
    theme(
  ...
  cowplot::plot_grid(toprow, middlerow, bottomrow, nrow = 3, rel_heights = c(2, 2, 1))
})
